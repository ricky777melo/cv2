...
test_images=json.load(open("test.json",'r'))
category_mapping=[]
...
def cal_iou(pred, gt):
    ...
    u_xmin = min(gt[0], pred[0])
    u_ymin = min(gt[1], pred[1])
    u_xmax = max(gt[2], pred[2])
    u_ymax = max(gt[3], pred[3])
    inter = max(0,(i_xmax-i_xmin)*(i_ymax-i_ymin))
    sum_area = max(0,(u_xmax-u_xmin)*(u_ymax-u_ymin))
    if sum_area!=0:
...
if __name__ == '__main__':
    ...
    gt_images = json.load(open('val.json'))
